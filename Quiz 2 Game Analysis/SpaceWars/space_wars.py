...
HoeffdingTree = joblib.load('./model/modelhTree.h5')
LABELS = {2: 'Hardcore Achiever', 3: 'Hardcore Killer', 1: 'Casual Achiever', 0: 'Casual Killer'}
LABELS2 = {1: 'Hardcore Achiever', 3: 'Hardcore Killer', 0: 'Casual Achiever', 2: 'Casual Killer'}
...
def prediction_user_type2(level, keyX_pressed_count, keyY_pressed_count, respawn_enemy_count, respawn_coin_count):
	global A0, A1
	a0 = statistics.mean(A0) if len(A0) else 0
	a1 = statistics.mean(A1) if len(A1) else 0
	a2 = coin_count
	...
	y = HoeffdingTree.predict(X)[0]
	return LABELS2.get(y)
...
def show_score(score, level, name, coin_count, font_size = 16, x=10, y=10, game_over=False, user_type=None ,user_type2=None):
	score_font = pygame.font.Font('freesansbold.ttf', font_size)
	...
	if user_type:
		user_type_text = score_font.render("(OLD)You are : " + str(user_type), True, (255, 255, 0))
	if user_type2:
		user_type_text2 = score_font.render("(NEW)You are : " + str(user_type2), True, (255, 255, 0))
	...
	if user_type2:
		y_pos = y_pos + 5 + font_size
		screen.blit(user_type_text2, (x, y_pos))
...
def show_game_over(screen_sizeX, screen_sizeY, score, high_score, coin_count, user_type,user_type2):
	...
	message_display_center('OLD : '+user_type, font_medium, yellow, int(screen_sizeX/2), int(screen_sizeY * 4/10))
	message_display_center('NEW : '+user_type2, font_medium, red, int(screen_sizeX/2), int(screen_sizeY * 5/10))
...
PLAYER_NAME	= 'Jeff'		#sws.PLAYER_NAME	# 'DAN'
...
def save_collection_data(level, keyX_pressed_count, keyY_pressed_count, respawn_enemy_count, respawn_coin_count):
	global A0,A1
	A00 = statistics.mean(A0) if len(A0) else 0
	A01 = statistics.mean(A1) if len(A1) else 0
	A2 = coin_count
	A3 = destroyed_enemy_count
	A4 = shots_count
	A5 = A4 - A3
	...
	A9 = respawn_enemy_count
	A10 = respawn_coin_count
	if sum([A00, A01, A2, A3, A4, A5]) != 0:
		with open("train_data.txt", "a") as file_object:
			file_object.write(",".join(map(str, [A00, A01, A2, A3, A4, A5, A6, A7, A8, A9, A10])))
			file_object.write("\n")
...
#			score += level_score_increase
		...
		if game_over:
			...
			user_type2 = prediction_user_type2(level, keyX_pressed_count, keyY_pressed_count, respawn_enemy_count, respawn_coin_count)
			show_game_over(screen_sizeX, screen_sizeY, score, session_high_score, coin_count, user_type,user_type2)
			...
			user_type = prediction_user_type(level, keyX_pressed_count, keyY_pressed_count, respawn_enemy_count, respawn_coin_count)
			user_type2 = prediction_user_type2(level, keyX_pressed_count, keyY_pressed_count, respawn_enemy_count, respawn_coin_count)
			show_score(score, level, PLAYER_NAME, coin_count, user_type=user_type,user_type2=user_type2)
			show_online_score()
